...
使用 `codex review` 原生命令直接审查，无需手动获取 diff 再嵌入 prompt。审查标准参考 `references/review_guidelines.md`。
...
# 审查未提交改动，附加自定义审查要求
codex review --uncommitted "只关注安全性和并发问题"
...
可通过 prompt 参数传入自定义审查要求，与 `references/review_guidelines.md` 中的标准配合使用。

**`codex review` vs `codex exec` 的区别：**
- `codex review` 是专用审查命令，自动获取 diff 内容，直接通过 Bash 调用，不走 `scripts/codex_exec.py`
